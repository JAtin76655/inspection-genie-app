...
import com.summonelec.inspectiongenie.databinding.ActivityManufactureLabelBinding
...
class ManufactureLabelActivity : AppCompatActivity() {
    lateinit var binding: ActivityManufactureLabelBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityManufactureLabelBinding.inflate(layoutInflater)
        setContentView(binding.root)
        progressAlert = ProgressAlert(this)
        binding.manufatureLabel.setOnClickListener(View.OnClickListener {
            showImageChooseAlert()
        })
        binding.btnNxt.setOnClickListener(View.OnClickListener {
            if (str_iv_package.equals(""))
            {
                Toast.makeText(this@ManufactureLabelActivity, "Please select Image", Toast.LENGTH_SHORT).show()
            }
            else
            {
    ...
    private fun showImageChooseAlert() {
        val dialog = Dialog(this@ManufactureLabelActivity)
    ...
    private val contract = registerForActivityResult(ActivityResultContracts.TakePicture()){
        ...
        try {
            ...
            val imagefilepath: String? = BitmapUtils.saveImage(this@ManufactureLabelActivity, bitmap!!)
            Log.e("Activity", "Pick from Camera::>>> "+imagefilepath)
            destination = File(imagefilepath)
            imgPath = destination!!.absolutePath
            binding.manufatureLabel.setImageBitmap(bitmap)
    ...
    private fun pickImageFromGallery() {
        if (ContextCompat.checkSelfPermission(this@ManufactureLabelActivity, Manifest.permission.READ_EXTERNAL_STORAGE)
        ...
        ActivityResultContracts.StartActivityForResult())
    {
        ...
        if (result.resultCode == AppCompatActivity.RESULT_OK) {
            ...
            try {
                ...
                val filePath = uriPathHelper.getPath(this@ManufactureLabelActivity, selectedImage!!)
        ...
        GlobalScope.launch {
            ...
            connection?.let {
                try {
                    ...
                    val inspection_id = SharedHelper.getKey(this@ManufactureLabelActivity, "inspection_id")
                    val insertQuery = "INSERT INTO manufacture_label (manufacture_label,inspection_id) VALUES ('"+str_iv_package+"','"+inspection_id+"')"
                    Log.e("insert query",insertQuery)
                    statement.executeUpdate(insertQuery)
                    Log.e("insert query status","Data inserted successfully!")
                    runOnUiThread {
                        progressAlert.dismissAlert()
                        val intent = Intent(this@ManufactureLabelActivity, DeterminepPartActivity::class.java)
